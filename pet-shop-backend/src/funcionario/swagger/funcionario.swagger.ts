...
export function ApiDocCriarFuncionario() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Cria um novo colaborador no sistema. **Regra Crítica:** É obrigatório enviar uma `senha` neste endpoint. O sistema irá criptografar a senha automaticamente antes de salvar no banco, permitindo que este funcionário faça Login na API.' 
    }),
    ApiResponse({ status: 201, description: 'Funcionário cadastrado e credenciais de acesso geradas com sucesso.' }),
    ApiResponse({ status: 400, description: 'Dados de entrada inválidos (ex: e-mail já existe no sistema ou senha não informada).' })
  );
}
...
export function ApiDocListarFuncionarios() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Retorna todos os funcionários cadastrados, seus cargos e dados de contato. Útil para o painel administrativo da loja.' 
    }),
