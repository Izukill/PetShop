...
export function ApiDocCriarCliente() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Registra um novo cliente no sistema. **Lembrete de Arquitetura:** Diferente dos funcionários, os clientes são salvos na tabela Pessoa sem a exigência de uma senha de acesso.' 
    }),
    ApiResponse({ status: 201, description: 'Cliente cadastrado com sucesso.' }),
    ApiResponse({ status: 400, description: 'Dados de entrada inválidos (ex: e-mail já cadastrado, formato de CPF incorreto).' })
  );
}
...
export function ApiDocListarClientes() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Retorna todos os clientes cadastrados. Útil para montar listas de transmissão de marketing ou consultar históricos no balcão.' 
    }),
