...
export class VendaService {
  ...
  async create(createVendaDto: CreateVendaDto) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        const totalServicos = await this.calcularTotalServicos(
          createVendaDto.servicoslookupId,
          tx,
        );
        ...
        const novaVenda = await tx.venda.create({
          data: {
            ...
            servicos:
              createVendaDto.servicoslookupId?.length > 0
                ? {
                    connect: createVendaDto.servicoslookupId.map((id) => ({
                      lookupId: id,
                    })),
                  }
                : undefined,
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2025') {
          throw new NotFoundException(
            `Erro de vínculo. Verifique se o Cliente, Produtos ou Serviços realmente existem.`,
          );
        }
      }
      throw error;
  ...
  async update(lookupId: string, updateVendaDto: UpdateVendaDto) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        //lógica de estoque
        if (
          updateVendaDto.status === VendaStatus.CANCELADA &&
          vendaAtual.status !== VendaStatus.CANCELADA
        ) {
          await this.executarLogicaCancelamento(lookupId, tx);
        } else if (
          updateVendaDto.status === VendaStatus.CONCLUIDA &&
          vendaAtual.status === VendaStatus.CANCELADA
        ) {
  ...
  private async calcularTotalServicos(
    servicosLookupId: string[] | undefined,
    tx: any,
  ): Promise<number> {
    ...
    if (servicos.length !== servicosLookupId.length) {
      throw new NotFoundException(
        'Um ou mais serviços informados não foram encontrados no banco de dados.',
      );
    }

    return servicos.reduce(
      (acc, servico) => acc + Number(servico.precoUnitario),
      0,
    );
  }
