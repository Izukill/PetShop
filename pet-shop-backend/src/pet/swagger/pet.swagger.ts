...
export function ApiDocCriarPet() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Adiciona um novo animalzinho ao sistema. **Regra importante:** Todo Pet deve estar obrigatoriamente vinculado ao ID de um Cliente (Tutor) existente.' 
    }),
    ApiResponse({ status: 201, description: 'Pet cadastrado com sucesso e vinculado ao tutor.' }),
    ApiResponse({ status: 400, description: 'Dados incompletos (ex: faltou o nome da espécie ou o ID do tutor).' }),
    ApiResponse({ status: 404, description: 'Cliente (Tutor) informado não foi encontrado.' })
  );
}
...
export function ApiDocListarPets() {
  return applyDecorators(
    ApiOperation({ 
      ...
      description: 'Retorna a lista de todos os animais da clínica/petshop, incluindo as informações básicas do seu respectivo tutor.' 
    }),
